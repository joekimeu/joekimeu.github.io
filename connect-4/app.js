...
function checkBoard() {
    for (let y = 0; y < winningArrays.length; y++) {
        ...
        if (
            ...
            highlightWinningSquares([square1, square2, square3, square4])
            disableBoard()
            return true
        } else if (
    ...
    // Check for draw
    const allSquares = [...squares].slice(0, -7) // Exclude bottom row
    if (allSquares.every(square => square.classList.contains('taken'))) {
...
function showMessage(message, duration = 2000) {
    ...
    if (duration) {
        setTimeout(() => {
            if (!checkBoard()) { // Only clear if there's no win/draw
                result.innerHTML = ''
            }
        }, duration)
    }
...
squares.forEach((square, i) => {
    square.onclick = () => {
        // If game is already won, prevent further moves
        if (result.innerHTML.includes('Wins') || result.innerHTML.includes('Draw')) {
            return
        }

        // Check if the square below is taken and current square is not
        if (squares[i + 7]?.classList.contains('taken') && !square.classList.contains('taken')) {
            if (currentPlayer === 1) {
            ...
            if (!checkBoard()) {
                // Play move sound or animation here if desired
            }
        } else if (!squares[i + 7]?.classList.contains('taken')) {
            showMessage('Cannot place piece here - try a lower position')
        }
    }
    ...
    square.onmouseover = () => {
        if (!square.classList.contains('taken') && 
            ...
            square.style.backgroundColor = 'var(--color-hover)'
        }
    }
